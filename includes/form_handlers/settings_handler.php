<?php 
	if(isset($_POST['update_details'])){
		...
		$email_ckeck = mysqli_query($con,"SELECT * FROM users WHERE email='$email'");
		$row = mysqli_fetch_array($email_ckeck);
	...
	if(md5($old_password) == $db_password){
		if($new_password_1 == $new_password_2){
			...
			if(strlen($new_password_1) <= 4){
				$password_message = "Sorry, your password must be greater than $ characters<br><br>";
			}else{
